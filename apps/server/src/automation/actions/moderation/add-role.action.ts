...
@Injectable()
export class AddRoleAction extends BaseAction {
  type = 'add_role';
  name = 'Add Role';
  ...
  async execute(context: WorkflowContext, config: any): Promise<ActionResult> {
    try {
      ...
      if (member.roles.cache.has(roleId)) {
        return this.success({ userId, roleId, message: 'User already has role' });
      }
      ...
      return this.failure(error.message || 'Failed to add role', { error: error.toString() });
    }
  }
  ...
  getConfigSchema(): any {
    return {
      ...
      properties: {
        ...
        reason: { type: 'string', description: 'Reason for role assignment' },
      },
      required: ['roleId'],
    };
  }
